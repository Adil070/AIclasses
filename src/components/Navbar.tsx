...
export default function Navbar({ settings }: { settings: SiteSettings }) {
  ...
  return (
    <nav className="glass fixed top-0 left-0 right-0 z-50 bg-surface/80 backdrop-blur-xl border-b border-ink/5">
      ...
      {menuOpen && (
        <div className="md:hidden bg-surface border-t border-ink/5 px-6 pt-3 pb-6 space-y-1">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
          ...
          <a
            href="#query"
            className="block bg-accent text-white text-center font-medium px-5 py-3 rounded-full text-sm mt-3"
            onClick={() => setMenuOpen(false)}
          >
            Enquire Now
          </a>
